...
os.chdir(rootdir)
outdir = '/data/prostate/postimages/AugmentedLabelsPercent7/'
...
for file in glob.glob('*T2.mha'):
    ...
    corelocnpy = np.zeros(np.shape(T2npy))
            ...
            for c in range(np.shape(corelocnpy)[2]):
                ...
                for i in range(len(rowlist)):
                    ...
                    if distance < coreradius:    # if the current pixel is in the space of the core
                        ...
                        if percent >= percentthreshold and path >= paththreshold:
                            corelocnpy[zslicei, r, c] += percent
    ...
    corelocnpy = corelocnpy.astype('uint16')
