...
outfile = '/Users/Leo/Documents/Stanford Sensitive/ProstateData/biopsypath/parsed4.csv'
df = pd.read_excel(file, sheet_name='Export Worksheet')

# single path pathreportdate
#file = '/Users/Leo/Documents/Stanford Sensitive/ProstateData/biopsypath/path_single.xlsx'
#outfile = '/Users/Leo/Documents/Stanford Sensitive/ProstateData/biopsypath/parsed_single.csv'
#df = pd.read_excel(file, sheet_name='Sheet1')

writecsv = True
includeerrormsgs = True

#### SET TRUE TO PRINT STUFF
debugmode = False
...
def getsystematiccoreindex(line):
    ...
    base = line.count('BASE') > 0 or line.count('BASAL') > 0
...
def gettargetedcoreloc(line):
    ...
    if re.search(r'lateral', line, re.IGNORECASE):
        lateral = True
    if re.search(r'medial', line, re.IGNORECASE):
        medial = True
    ...
    if lateral:
        corelocation = corelocation + 'lateral '
    if medial:
        corelocation = corelocation + 'medial '
    if mid:
...
for index, row in df.iterrows():
    if 1668 == 1668:
        ...
        # skip path reports 2013 and before
        #print(accession[4:6])
        if int(accession[4:6]) <= 13:    #SHS-13-   year is 4 - 6
            continue

        report = df['REPORT'][index]

        # skips if non prostate biopsy report
        reportlower = report.lower()
        exclusions = ['turp', 'prostatectomy', 'lymph node', 'transurethral', 'voided', 'bladder biops', 'bladder tumor']
        if any(word in reportlower for word in exclusions):
            continue
        ...
        if len(errormsgs) != 0 and not ('ADDENDUM' in errormsgs):
        ...
        ###########################################################
        # Parses DIAGNOSIS BLOCK  into individual lines
        # and stores them in an array _specimenlines_

        specimenlines0 = re.split(r'(\n[A-Z]+[.])', diagnosisblock)
        specimenlines0 = specimenlines0[1:]

        if len(specimenlines0) == 0:
            continue

        specimenlines = []
        if len(specimenlines0) % 2 == 1:
            n = int((len(specimenlines0) - 1) / 2)   # 2n + 1 --> n + 1

            specimenlines.append(specimenlines0[0])
            for ii in range(n):
                specimenlines.append(specimenlines0[2*ii + 1] + specimenlines0[2*ii + 2])
        else:
            n = int(len(specimenlines0) / 2)   # 2n --> n

            for ii in range(n):
                specimenlines.append(specimenlines0[2*ii] + specimenlines0[2*ii + 1])

        # takes the last 3 lines
        lastline = specimenlines[-1]
        if len(lastline.splitlines()) <= 3:
            lastline2 = lastline
        else:
            lastline2 = lastline.splitlines()[0] + lastline.splitlines()[1] + lastline.splitlines()[2]

        specimenlines[-1] = lastline2
        ...
        for i, line in enumerate(specimenlines):
            if re.search(r'\d{1,2}\s*-\s*\d', line) and not re.search(r'level.{0,5}\d\s*-\s*\d', line.lower()):
                firsttargetindex = i
                break
        ...
        if len(systematicindices) != len(set(systematicindices)):
            ###########################################################
            # If DIANGOSIS block has conflicts, will try to parse
            # SPECIMEN BLOCK  into individual lines
            # and stores them in an array _specimenlines_
    ...
    #    print(grosslines)
        ...
        if debugmode:
            print('diagnosis block: ', diagnosisblock)
            print('diagnosislines: ', diagnosislines)
            print('systematic cores: ', systematicindices)
            print('core lengths: ', corelengths)


        if len(errormsgs) > 0 and not ('core length' in errormsgs) and not ('ADDENDUM' in errormsgs):
            primary = secondary = total = coreindex = corelength = -1
            PIRADS = date = systar = percentcore = pattern4 = pattern5 = corelocation = secondarychar = ''
            parseddf.append(dict(Index=index, MRN=chartID, PathDate=date, Accession=accession, SysOrTar=systar, PIRADS=PIRADS,
                    CoreName=coreindex, CoreLocation=corelocation, Primary=primary, Secondary=secondary,
                    PercentCore=percentcore, CoreLength=corelength, PercentPattern4=pattern4,
                    PercentPattern5=pattern5, Other=secondarychar, ErrorMsg=errormsgs))
        else:
            ...
            for i, line in enumerate(systematicdiagnosislines):
                ...
                try:
                    ...
                    corelength = ''
                    errormsgs2 = errormsgs + 'MISSING core length for SYS CORE ' + str(i + 1) + '; '
                ...
                parseddf.append(dict(Index=index, MRN=chartID, PathDate=date, Accession=accession, SysOrTar=systar, PIRADS=PIRADS,
                        ...
                        PercentPattern5=pattern5, Other=secondarychar, ErrorMsg=errormsgs2))
            ...
            for i, line in enumerate(targeteddiagnosislines):
                ...
                if len(coreindex) == 0 and len(coreindex2) > 0:
                    ...
                    if coreindex != coreindex2:
                        errormsgs2 = errormsgs2 + 'INCONSISTENT TARGETED CORE NAME ' + str (i+1)
                        #print('>0 ' + accession + '  ' + coreindex + '  ' + coreindex2)
                        #print(line.encode('utf-8'))
                        #print(i)
                        #print(targetedlines)
                ...
                try:
                    ...
                    errormsgs2 = errormsgs2 + 'MISSING core length for TARGET CORE ' + str(i + 1) + '; '
...
cols = ['Index', 'MRN', 'PathDate', 'Accession', 'SysOrTar', 'ErrorMsg', 'NumSysCores', 'CoreName', 'PIRADS', 'CoreLocation', \
...
if includeerrormsgs == False:
    # GET RID OF 'DUPLICATE SYSTEMATIC CORES', 'NO _DIAGNOSIS_', 'LEFT AND RIGHT'
    parseddf = parseddf[~parseddf.ErrorMsg.str.contains('DUPLICATE SYSTEMATIC CORES')]
    parseddf = parseddf[~parseddf.ErrorMsg.str.contains('NO _DIAGNOSIS')]
    parseddf = parseddf[~parseddf.ErrorMsg.str.contains('LEFT AND RIGHT')]
