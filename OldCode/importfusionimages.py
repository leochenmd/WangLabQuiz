...
errorpath = os.path.join(outdir, 'fusionimageserrors.xls')
    ...
    #    series = seriesIDs[1]
        try:
            ...
            if serieslabel != 'DCE':
                ...
                sitk.WriteImage(image, os.path.join(OUTPUT_DIR, mhaname + ".mha"))

        except:
            print('Error in reading ' + str(datepath) + '/' + str(series))
            errormsgs.append('Error in reading ' + str(series))

outDF = pd.DataFrame(data = [accession_list, mrdate_list, seriesname_list, serieslabel_list, numslices_list, dimensions_list, pixelspacing_list, mhanames_list])
